...
class Demo(BaseDemo):
    ...
    def test_unsupervised(self):
        ...
        for epoch in range(self.test_epoch):
            ...
            if self.display:
                ...
                self.visualizer.visualize_result(im_input, im_output, im_pred, flow, gt_motion, unocclude, occlude, 'test_%d.png' % epoch)
    ...
    def test_gt_unsupervised(self):
        ...
        for epoch in range(self.test_epoch):
            ...
            if self.display:
                flow = self.motion2flow(m_mask)
                self.visualizer.visualize_result(im_input, im_output, im_pred, flow, gt_motion, unocclude, occlude, 'test_gt.png')
        test_loss = numpy.mean(numpy.asarray(test_loss))
        base_loss = numpy.mean(numpy.asarray(base_loss))
        improve_loss = base_loss - test_loss
        improve_percent = improve_loss / (base_loss + 1e-5)
