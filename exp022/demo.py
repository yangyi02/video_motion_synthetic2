...
class Demo(BaseDemo):
    ...
    def train_unsupervised(self):
        ...
        for epoch in range(self.train_epoch):
            ...
            a = torch.abs(m_mask[:, :, :-1, :] - m_mask[:, :, 1:, :]).sum(1) * F.relu(1 - torch.abs(depth[:, :, :-1, :] - depth[:, :, 1:, :]))
            b = torch.abs(m_mask[:, :, :, :-1] - m_mask[:, :, :, 1:]).sum(1) * F.relu(1 - torch.abs(depth[:, :, :, :-1] - depth[:, :, :, 1:]))
    ...
    def test_unsupervised(self):
        ...
        for epoch in range(self.test_epoch):
            im, motion, _, gt_depth = self.data.get_next_batch(self.data.test_images)
            im_input = im[:, :-1, :, :, :].reshape(self.batch_size, -1, self.im_size, self.im_size)
            im_output = im[:, -1, :, :, :]
            gt_motion = motion[:, -2, :, :, :]
            gt_depth = gt_depth[:, -2, :, :, :]
