...
class SyntheticDataBidirect(object):
    ...
    def generate_data(self, src_image, src_mask):
        ...
        src_motion_f, m_x_f, m_y_f, src_motion_b, m_x_b, m_y_b = \
            self.generate_motion(self.num_objects, src_mask)
        # move foreground forward
        fg_im_f, fg_motion_f, fg_motion_f_r, fg_mask_f = \
            self.move_foreground(src_image, src_mask, src_motion_f, src_motion_b, m_x_f, m_y_f)
        # move foreground backward
        fg_im_b, fg_motion_b, fg_motion_b_r, fg_mask_b = \
            self.move_foreground(src_image, src_mask, src_motion_b, src_motion_f, m_x_b, m_y_b)
        # generate background
        src_bg = numpy.random.rand(batch_size, 3, im_size, im_size) * self.bg_noise
        # generate background motion
        if self.bg_move:
            src_motion_f, m_x_f, m_y_f, src_motion_b, m_x_b, m_y_b = self.generate_motion(1)
        else:
            src_motion_f = m_dict[(0, 0)] * numpy.ones((1, batch_size, 1, im_size, im_size))
            m_x_f = numpy.zeros((1, batch_size)).astype(int)
            m_y_f = numpy.zeros((1, batch_size)).astype(int)
            src_motion_b = m_dict[(0, 0)] * numpy.ones((1, batch_size, 1, im_size, im_size))
            m_x_b = numpy.zeros((1, batch_size)).astype(int)
            m_y_b = numpy.zeros((1, batch_size)).astype(int)
        # move background
        bg_im_f, bg_motion_f, bg_motion_f_r = \
            self.move_background(src_bg, src_motion_f, src_motion_b, m_x_f, m_y_f)
        bg_im_b, bg_motion_b, bg_motion_b_r = \
            self.move_background(src_bg, src_motion_b, src_motion_f, m_x_b, m_y_b)
        # merge foreground and background, merge foreground motion and background motion
        mask = fg_mask_f == 0
        fg_motion_f[mask] = bg_motion_f[mask]
        fg_motion_f_r[mask] = bg_motion_f_r[mask]
        im_mask = numpy.concatenate((mask, mask, mask), 2)
        fg_im_f[im_mask] = bg_im_f[im_mask]
        mask = fg_mask_b == 0
        fg_motion_b[mask] = bg_motion_b[mask]
        fg_motion_b_r[mask] = bg_motion_b_r[mask]
        im_mask = numpy.concatenate((mask, mask, mask), 2)
        fg_im_b[im_mask] = bg_im_b[im_mask]
        # merge forward and backward frames and motions
        im, motion, motion_r = self.merge_forward_backward(fg_im_f, fg_motion_f, fg_motion_f_r,
                                                           fg_im_b, fg_motion_b, fg_motion_b_r)
        # swap axes between batch size and frame
        im, motion, motion_r = im.swapaxes(0, 1), motion.swapaxes(0, 1), motion_r.swapaxes(0, 1)
        return im, motion.astype(int), motion_r.astype(int)
    ...
    def generate_motion(self, num_objects, src_mask=None):
        ...
        src_motion_f = m_dict[(0, 0)] * numpy.ones((num_objects, batch_size, 1, im_size, im_size))
        src_motion_b = m_dict[(0, 0)] * numpy.ones((num_objects, batch_size, 1, im_size, im_size))
        if src_mask is None:
            src_mask = numpy.ones((num_objects, batch_size, 1, im_size, im_size))
        for i in range(num_objects):
            for j in range(batch_size):
                mask = src_mask[i, j, :, :, :] > 0
                src_motion_f[i, j, mask] = m_label[i, j]
                src_motion_b[i, j, mask] = m_dict[(m_x_b[i, j], m_y_b[i, j])]
        return src_motion_f, m_x_f, m_y_f, src_motion_b, m_x_b, m_y_b

    def move_foreground(self, src_image, src_mask, src_motion, src_motion_r, m_x, m_y):
        ...
        motion_r = numpy.zeros((num_frame, batch_size, 1, im_size, im_size))
        mask = numpy.zeros((num_frame, batch_size, 1, im_size, im_size))
        for i in range(num_frame):
            im[i, ...], motion[i, ...], motion_r[i, ...], mask[i, ...] = \
                self.merge_objects(src_image, src_motion, src_motion_r, src_mask)
        ...
        return im, motion, motion_r, mask

    def merge_objects(self, src_image, src_motion, src_motion_r, src_mask):
        ...
        motion_r = numpy.zeros((batch_size, 1, im_size, im_size))
        ...
        for i in range(num_objects):
            ...
            motion_r[zero_mask] = src_motion_r[i, zero_mask]
    ...
    def move_background(self, src_image, src_motion, src_motion_r, m_x, m_y):
        batch_size, num_frame, im_size = self.batch_size, self.num_frame_one_direction, self.im_size
        im = numpy.zeros((num_frame, batch_size, 3, im_size, im_size))
        motion = numpy.zeros((num_frame, batch_size, 1, im_size, im_size))
        motion_r = numpy.zeros((num_frame, batch_size, 1, im_size, im_size))
    ...
    @staticmethod
    def merge_forward_backward(im_f, motion_f, motion_f_r, im_b, motion_b, motion_b_r):
        im_f = im_f[1:, ...]
        im_b = im_b[::-1, ...]
    ...
    def display(self, im, motion, motion_r):
        ...
        for i in range(num_frame):
            ...
            flow = self.visualizer.label2motion(motion_r[0, i, :, :, :].squeeze())
            optical_flow = flowlib.visualize_flow(flow, self.m_range)
