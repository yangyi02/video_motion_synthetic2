...
class Visualizer(BaseVisualizer):
    ...
    def visualize_result_bidirect(self, im_input_f, im_input_b, im_output, im_pred, pred_motion_f,
                                  gt_motion_f, depth_f, gt_depth_f, appear_f, conflict_f, attn_f,
                                  pred_motion_b, gt_motion_b, depth_b, gt_depth_b, appear_b,
                                  conflict_b, attn_b, file_name='tmp.png'):
        width, height = self.get_img_size(4, max(self.num_frame + 1, 7))
        img = numpy.ones((height, width, 3))
        prev_im = None
        for i in range(self.num_inputs):
            curr_im = im_input_f[0, i*3:(i+1)*3, :, :].cpu().data.numpy().transpose(1, 2, 0)
        ...
        for i in range(self.num_inputs):
            curr_im = im_input_b[0, i * 3:(i + 1) * 3, :, :].cpu().data.numpy().transpose(1, 2, 0)
