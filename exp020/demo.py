...
class Demo(BaseDemo):
    ...
    def train_unsupervised(self):
        ...
        for epoch in range(self.train_epoch):
            ...
            im_pred, m_mask_f, depth_f, appear_f, conflict_f, attn_f, m_mask_b, depth_b, appear_b, conflict_b, attn_b, pred_depth, depth_out = \
                self.model(im_input_f, im_input_b, im_output)
            loss = torch.abs(im_pred - im_output).sum()
            loss = loss + torch.abs(pred_depth - depth_out).sum()
            loss.backward()
            optimizer.step()
    ...
    def test_unsupervised(self):
        ...
        for epoch in range(self.test_epoch):
            im, motion, motion_r, _, _, gt_depth = self.data.get_next_batch(self.data.test_images)
            im_input_f = im[:, :self.num_inputs, :, :, :].reshape(
                self.batch_size, -1, self.im_size, self.im_size)
            im_input_b = im[:, :self.num_inputs:-1, :, :, :].reshape(
                self.batch_size, -1, self.im_size, self.im_size)
            ...
            if self.display:
                self.visualizer.visualize_result_bidirect(im_input_f, im_input_b, im_output,
                                                          im_pred, flow_f, gt_motion_f, depth_f,
                                                          gt_depth_f, appear_f, conflict_f, attn_f,
                                                          flow_b, gt_motion_b, depth_b, gt_depth_b,
                                                          appear_b, conflict_b, attn_b,
                                                          'test_%d.png' % epoch)
    ...
    def test_gt_unsupervised(self):
        ...
        for epoch in range(self.test_epoch):
            ...
            gt_depth_f = gt_depth[:, self.num_inputs-1, :, :, :]
            gt_depth_b = gt_depth[:, self.num_inputs+1, :, :, :]
            gt_depth_out = gt_depth[:, self.num_inputs, :, :, :]
            im_input_f = Variable(torch.from_numpy(im_input_f).float())
            im_input_b = Variable(torch.from_numpy(im_input_b).float())
            im_output = Variable(torch.from_numpy(im_output).float())
            gt_motion_f = Variable(torch.from_numpy(gt_motion_f).float())
            ...
            im_pred, m_mask_f, depth_f, appear_f, conflict_f, attn_f, m_mask_b, depth_b, appear_b, conflict_b, attn_b, pred_depth, depth_out = \
                self.model_gt(im_input_f, im_input_b, gt_motion_label_f, gt_motion_label_b, gt_depth_f, gt_depth_b, gt_depth_out)
            loss = torch.abs(im_pred - im_output).sum()
            loss = loss + torch.abs(pred_depth - depth_out).sum()
